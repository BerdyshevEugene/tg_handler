...
from service.reminder import add_reminder, list_reminders, delete_reminder_by_index, parse_indices
...
async def handle_service_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    message = 'обновления: \n- исправлена ошибка при ежедневной рассылке задач'
...
async def sendlocation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.message.reply_text('пожалуйста, отправьте мне вашу геопозицию, чтобы я мог отправлять вам прогноз погоды')
...
async def location(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text('сохранено! Теперь я буду отправлять вам прогноз погоды по указанной геопозиции')
    ...
    try:
        ...
        if indices:
            ...
            if successful_deletions:
                await update.message.reply_text(f'удалены напоминания под номерами: {", ".join(str(idx + 1) for idx in successful_deletions)}')
            else:
